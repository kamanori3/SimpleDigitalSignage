...
| 項目 | 内容 |
|------|------|
| 製品名 | 簡単！シンプル！サイネージ（exe は `PdfSignage.exe`） |
...
| 配布形態 | ポータブル（フォルダ一式をコピー、インストーラーなし）。現場には `PdfSignage` のみ。`PdfSignage.Issuer` は同梱しない |
| 課金 | `PdfSignage.Licensing`（net8.0）。公開鍵埋め込みの ECDSA P-256。試用開始日は HKCU + LocalAppData（[ADR 0013](adr/0013-試用開始日はPC側に残す.md)） |
...
flowchart TB
  ...
  subgraph License["課金 (PdfSignage.Licensing / Services/)"]
    LS[LicenseService]
    TM[TrialMarkerStore]
  end
...
運用終了は 2 系統ある。スケジュールと管理画面の「アプリを終了して PC を電源オフ」は `AppExit.Execute`（先に電源オフ、そのあとプロセス終了）。管理画面の「アプリ終了」は `AppExit.ExitProcess`（電源オフしない）。× 閉じは表示モードへ戻るだけで終了しない。

| 経路 | 実装 |
|------|------|
| 管理画面「表示モードに戻る」 / × 閉じ / Ctrl+Shift+M | `MainWindow.ExitAdminMode()` または `OnAdminWindowClosed()`。キオスク表示に戻る |
| 管理画面「アプリ終了」 | 確認ダイアログ → `AdminViewModel.ExitApplicationRequested` → `MainWindow.ExitApplication()` |
| 管理画面「アプリを終了して PC を電源オフ」 | 確認ダイアログ → `AdminViewModel.ExitAndPowerOffRequested` → `MainWindow.ExitAndPowerOff()` |
| スケジュール（終了時刻） | `ScheduleService.ExitRequested` → `MainWindow.OnScheduledExit()`（確認なし・電源オフあり） |
| 未処理例外 | `AppSelfRestartService` が別プロセスを起動してから `Shutdown()`。**電源オフしない** |
| 二重起動 | Mutex 取得失敗で `Shutdown()`。**電源オフしない** |
...
`MainWindow.EnterAdminMode()` のたびに、実行中の監視フォルダを再スキャンする。キオスク再生中には出さない。既読管理はせず、入場のたびに出し直す。
...
| `AppExitTime` | `string?` | `null` | `HH:mm`。null = 無効。到達時にアプリ終了＋PC 電源オフ |
| `PcShutdownTime` | `string?` | （読み取り専用） | 旧設定互換。読み込み後に `SettingsMigration` が `AppExitTime` へ統合して null にする |
...
ログフォルダは **この PC の `%LocalAppData%\PdfSignage\logs`** に固定する（[ADR 0014](adr/0014-ログをこのPCのLocalAppDataに置く.md)）。監視フォルダ（共有・USB を含む）には連動しない。`settings.json` の読み込み失敗だけは exe 横の `settings_load_error.log`。
...
| `ScheduleService` | 30 秒間隔で終了時刻を監視 | 「跨いだ瞬間」でのみ発火。日付跨ぎでは `DateRolledOver` も発火 |
| `AppExit` | 運用終了の共通経路 | `Execute` は電源オフ付き。`ExitProcess` はアプリのみ。自己再起動では使わない |
| `SystemPower` | `shutdown /s /t 0` を実行 | ポリシーで失敗しうる。失敗はログのうえアプリのみ終了 |
| `SettingsMigration` | 旧 `pcShutdownTime` を `appExitTime` へ統合 | 両方あるときは `appExitTime` を残す |
...
キー形式は `PDS1.{payloadB64url}.{sigB64url}`。ペイロードは `v` `n` `org` `exp`（yyyy-MM-dd）`plan`（`standard` / `site`）。検証は埋め込み公開鍵のみ（`AccessKeyPublicKey`）。秘密鍵は `secrets/`（git 対象外）。発行は `PdfSignage.Issuer`。

試用開始日は exe 横に置かない。`TrialMarkerStore` が `HKCU\Software\PdfSignage\TrialStart` と `%LocalAppData%\PdfSignage\trial.dat` へ XOR 難読化して二重書きし、欠け側へ書き戻す。両方無い／壊れているとその日が新しい初日になる。

帯の文言は `LicenseService.RenewalBannerMessage`（試用切れも同じ文面。方針どおり）。失敗してもキオスクは出す。
...
| `AdminViewModel` | 設定の編集・検証・保存、アクセスキー適用、ライセンス表示。タイトルバーに製品名とバージョン |
...
| `TrialMarkerStore` | HKCU と LocalAppData への試用開始日の読み書き（[ADR 0013](adr/0013-試用開始日はPC側に残す.md)） |
...
| `ScheduleService` `ScheduleTimeHelper` `AppExit` `SystemPower` `SettingsMigration` | 日次スケジュールと運用終了 |
...
入口は `LicenseStateEvaluator` と `LicenseService`。試用残存は `TrialMarkerStore` / `TrialMarkerResolver`。帯の出し入れは `MainViewModel.RefreshLicenseBanner`。キー形式は `AccessKeyCodec`。公開鍵を差し替えると既存キーはすべて無効になる。ドキュメントは操作ガイド・運用ガイド・受け入れテスト・課金方針・[ADR 0013](adr/0013-試用開始日はPC側に残す.md)。
...
| 自動テスト | UI・フォルダ監視・`TrialMarkerStore` の実 I/O は無い。退行の手動確認は [受け入れテスト.md](受け入れテスト.md)。純粋ロジック（`DisplayDurationParser`（秒数・期限） `ContentFileProbe` `NaturalStringComparer` `ScheduleTimeHelper` `SettingsValidator` `PathHelper` `WatchFolderLocationPolicy` アクセスキー／課金状態）は `PdfSignage.Tests` でカバー済み |
