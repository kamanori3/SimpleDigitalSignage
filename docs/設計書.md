...
| プロパティ | 型 | 既定値 | 備考 |
|-----------|-----|--------|------|
| `WatchFolderPath` | `string` | `D:\Signage` | Drive URL 指定時は表示に使わない |
| `GoogleDriveFolderUrl` | `string` | `""` | 空でなければ公開 Drive フォルダを同期 |
| `GoogleDriveApiKey` | `string` | `""` | Drive URL 指定時は必須 |
| `GoogleDriveSyncIntervalMinutes` | `int` | `5` | 1〜60 へクランプ |
...
```
ResolveContentFolder(設定)
  GoogleDriveFolderUrl がある           → {exe}/DriveCache を作成して使用
  それ以外                              → ResolveWatchFolderPath(監視フォルダ)
```
...
ログフォルダは**監視フォルダの親階層 + `logs`**（`D:\Signage` → `D:\logs`）。Drive 運用時は `DriveCache` の親（exe フォルダ）+ `logs`。設定上のドライブが存在しない場合だけ、実際に解決された監視フォルダを基準にする。開発機に `D:` が無くても動くようにするための分岐。
...
AdminViewModel.TrySaveAndApply()
  ...
  → ApplicationContext.ApplySettings()   解決済みパス・ログ出力先・Drive 同期を差し替え
...
| `DriveFolderSyncService` | 公開 Drive フォルダ直下を `DriveCache` へ定期同期（[ADR 0009](adr/0009-google-driveは公開フォルダをキャッシュ同期する.md)） |
| `DriveFolderUrlParser` | フォルダ URL から ID を取り出す |
