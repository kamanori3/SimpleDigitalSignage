...
| 課金 | `PdfSignage.Licensing`（net8.0）。公開鍵埋め込みの ECDSA P-256。試用開始日は HKLM / ProgramData + HKCU / LocalAppData（[ADR 0013](adr/0013-試用開始日はPC側に残す.md)、[ADR 0015](adr/0015-試用開始日の片方はマシン権限に置く.md)） |
...
試用開始日は exe 横に置かない。`TrialMarkerStore` がマシン領域（`HKLM\Software\PdfSignage\TrialStart` と `%ProgramData%\PdfSignage\trial.dat`）とユーザー領域（`HKCU\Software\PdfSignage\TrialStart` と `%LocalAppData%\PdfSignage\trial.dat`）へ XOR 難読化して書き、欠け側へ書き戻す。HKLM への書き込みは管理者昇格時だけ（失敗しても起動は続ける）。すべて無い／壊れているとその日が新しい初日になる。
...
入口は `LicenseStateEvaluator` と `LicenseService`。試用残存は `TrialMarkerStore` / `TrialMarkerResolver`。帯の出し入れは `MainViewModel.RefreshLicenseBanner`。キー形式は `AccessKeyCodec`。公開鍵を差し替えると既存キーはすべて無効になる。ドキュメントは操作ガイド・運用ガイド・受け入れテスト・課金方針・[ADR 0013](adr/0013-試用開始日はPC側に残す.md)・[ADR 0015](adr/0015-試用開始日の片方はマシン権限に置く.md)。
