...
ログフォルダは**監視フォルダの親階層 + `logs`**（`D:\Signage` → `D:\logs`）。設定上のドライブが存在しない場合、またはネットワークパスを拒否してフォールバックした場合は、実際に解決された監視フォルダを基準にする。開発機に `D:` が無くても動くようにするための分岐。
...
| `AdminViewModel` | 設定の編集・検証・保存、アクセスキー適用、ライセンス表示。ウィンドウタイトルにバージョンを表示 |
